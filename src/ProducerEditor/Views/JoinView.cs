...
namespace ProducerEditor.Views
{
	...
	{
		public JoinView(Controller controller, Producer[] producers)
		{
...
/*									if (args.KeyCode == Keys.Enter)
			...
			button.Click += (sender, args) => DoSearch(producers, controller, text, producersTable);
		...
		{
			var producers = controller.SearchProducer(text.Text).Where(p => source.Any(s => s.Id != p.Id)).ToList();
